...
def dqn_feature(**kwargs):
    ...
    # run_evals(DQNAgent(config), "data/DQNAgent-CartPole-v0--run-0-20000")
...
def dqn_pixel(**kwargs):
    ...
    config.network_fn = lambda: nn(config.action_dim, NatureConvBody(in_channels=config.history_length))
...
def n_step_dqn_feature(**kwargs):
    ...
    config.num_workers = 5
    config.optimizer_fn = lambda params: torch.optim.RMSprop(params, 0.0005)
    config.network_fn = lambda: nn(config.action_dim, FCBody(config.state_dim))
    config.random_action_prob = LinearSchedule(1.0, 0.1, 1e4)
    config.discount = 0.95
    config.target_network_update_freq = 50
    config.rollout_length = 5
...
def n_step_dqn_pixel(**kwargs):
    ...
    config.num_workers = 16
    config.optimizer_fn = lambda params: torch.optim.RMSprop(params, lr=1e-4, alpha=0.99, eps=1e-5)
    config.network_fn = lambda: nn(config.action_dim, NatureConvBody())
