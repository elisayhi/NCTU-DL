...
/**
 * output streams
 * to enable debugging (more output), just change the line to 'ostream& debug = cout;'
 */
ostream& info = cout;
ostream& error = cerr;
ostream& debug = *(new ofstream);
...
public:
	...
	void popup(int p, int n) {
		set(space[p], n);
	}

	/**
	 * count how many grids are empty
	 */
	int cnt_empty() {
		int empty_num = 0;
		for(int i=0; i<16; i++)
...
public:
	 ...
	 */
	virtual void dump(const board& b, ostream& out = info) const {
		out << b << "estimate = " << estimate(b) << endl;
	}

	friend ostream& operator <<(ostream& out, const feature& w) {
		string name = w.name();
		int len = name.length();
	...
	friend istream& operator >>(istream& in, feature& w) {
		string name;
		int len = 0;
		...
		in.read(&name[0], len);
		if (name != w.name()) {
			error << "unexpected feature: " << name << " (" << w.name() << " is expected)" << endl;
			exit(1);
		}
		float* weight = w.weight;
		size_t size;
		...
		if (size != w.size()) {
			...
			error << " (" << w.size() << " is expected)" << endl;
			exit(1);
		}
		in.read(reinterpret_cast<char*>(weight), sizeof(float) * size);
		if (!in) {
			error << "unexpected end of binary" << endl;
			exit(1);
		}
		return in;
	}
...
protected:
	static float* alloc(size_t num) {
		...
		try {
			total += num;
			if (total > limit) throw bad_alloc();
			return new float[num]();
		} catch (bad_alloc&) {
			error << "memory limit exceeded" << endl;
			exit(-1);
		}
		return nullptr;
	}
...
public:
	pattern(const vector<int>& p, int iso = 8) : feature(1 << (p.size() * 4)), iso_last(iso) {
		if (p.empty()) {
			error << "no pattern defined" << endl;
			exit(1);
		}
...
public:
	...
	virtual float estimate(const board& b) const {
		...
		return value;	// after-state
		//return value / b.cnt_empty();	//state
	}
	...
	virtual string name() const {
		return to_string(isomorphic[0].size()) + "-tuple pattern " + nameof(isomorphic[0]);
	}
...
public:
	 ...
	 */
	void dump(const board& b, ostream& out = info) const {
		for (int i = 0; i < iso_last; i++) {
			out << "#" << i << ":" << nameof(isomorphic[i]) << "(";
			size_t index = indexof(isomorphic[i], b);
			for (size_t i = 0; i < isomorphic[i].size(); i++) {
				out << hex << ((index >> (4 * i)) & 0x0f);
			}
			out << dec << ") = " << operator[](index) << endl;
		}
	}

protected:

	size_t indexof(const vector<int>& patt, const board& b) const {
		size_t index = 0;
	...
	string nameof(const vector<int>& patt) const {
		stringstream ss;
		ss << hex;
		copy(patt.cbegin(), patt.cend(), ostream_iterator<int>(ss, ""));
		return ss.str();
	}

	array<vector<int>, 8> isomorphic;
	int iso_last;
};
...
public:
	state(int opcode = -1)
		: opcode(opcode), score(-1), esti(-numeric_limits<float>::max()) {}
	state(const board& b, int opcode = -1)
		: opcode(opcode), score(-1), esti(-numeric_limits<float>::max()) { assign(b); }
...
public:
	...
	bool operator < (const state& s) const {
		if (before != s.before) throw invalid_argument("state::operator<");
		return esti < s.esti;
	}
...
public:
    ...
    friend ostream& operator <<(ostream& out, const state& st) {
		...
		if (st.is_valid()) {
			out << ", value = " << st.esti << endl << st.after;
		} else {
			out << " (invalid)" << endl;
		}
		return out;
...
public:
	 ...
	 *
	 * note that feats is vector<feature*>,
	...
	float estimate(const board& b) const {
		debug << "estimate " << endl << b;
		float value = 0;
		for (feature* feat : feats) {
			//cout << "estimate" << feat->estimate(b) <<' ' << b.cnt_empty()<< endl;
	...
	state select_best_move(const board& b) const {
		...
		for (state* move = after; move != after + 4; move++) {
			if (move->assign(b)) {
				...
				move->set_value(-numeric_limits<float>::max());
			}
			debug << "test " << *move;
		}
		//cout << *best << endl;
	 ...
	 */
	float sum_all_possible_next_state(const state& best_s) const {
		board* after_b = new board(best_s.after_state());

		float ttl_value = 0.0;	// sum(p*v)
		int empty_num = after_b->cnt_empty();
		...
		for (int i=0; i<empty_num; i++) {
			...
			b = new board(*after_b);
			b->popup(i, 2);
			ttl_value += (estimate(*b)) / empty_num * 0.1;
			delete b;
		}
		delete after_b;
	 ...
	 */
	// void update_episode(vector<state>& path, float alpha = 0.1) const {
	// 	float exact = 0;
	// 	for (path.pop_back() /* terminal state */; path.size(); path.pop_back()) {
	// 		state& move = path.back();
	// 		float error = exact - (move.value() - move.reward());
	// 		debug << "update error = " << error << " for after state" << endl << move.after_state();
	...
	void update_episode(vector<state>& path, float alpha = 0.1) const {
		...
		for (path.pop_back() /* terminal state */; path.size(); path.pop_back()) {
			...
			debug << "update error = " << error << " for after state" << endl << move.before_state();
	...
	void make_statistic(size_t n, const board& b, int score, int unit = 1000) {
		...
		if (n % unit == 0) { // show the training process
			...
			for (int t = 1, c = 0; c < unit; c += stat[t++]) {
				...
				int accu = accumulate(stat + t, stat + 16, 0);
				info << "\t" << ((1 << t) & -2u) << "\t" << (accu * coef) << "%";
				info << "\t(" << (stat[t] * coef) << "%)" << endl;
			}
			scores.clear();
			maxtile.clear();
		}
	...
	void load(const string& path) {
		...
		if (in.is_open()) {
			...
			if (size != feats.size()) {
				error << "unexpected feature count: " << size << " (" << feats.size() << " is expected)" << endl;
				exit(1);
			}
			for (feature* feat : feats) {
				in >> *feat;
				info << feat->name() << " is loaded from " << path << endl;
			}
			in.close();
		}
	}
